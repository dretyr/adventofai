...
def preprocess_mappings(ranges):
    max_num = max(max(src_end, dest_start + (src_end - src_start)) for src_start, src_end, dest_start in ranges)
    lookup = list(range(max_num + 1))
...
def convert_range(start, length, lookup):
    end = min(start + length, len(lookup))
    converted_values = lookup[start:end]
    return min(converted_values) if converted_values else start
